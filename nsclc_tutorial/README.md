...
1. `notebooks/tutorial-figures.ipynb` In header `Figure 5B`, the dataframe is averaged across all ROIs and an `sns.clustermap()` is constructed. In `Figure 5C`, each ROI-level spatial interaction matrix was flattened, and patient survival data is merged. Log-rank tests were computed for each pairwise interaction in 953 eligible patients, and FDR-adjusted p-values are reported.